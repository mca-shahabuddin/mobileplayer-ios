...
class BuyOverlayViewController: MobilePlayerOverlayViewController {
  ...
  init(product: Product) {
    ...
    descriptionLabel.font = UIFont.systemFont(ofSize: 8, weight: UIFont.Weight.heavy)
    containerView.contentView.addSubview(descriptionLabel)
    //containerView.addSubview(descriptionLabel)
    nameLabel.text = product.name
    nameLabel.textColor = UIColor.white
    nameLabel.font = UIFont.systemFont(ofSize: 7, weight: UIFont.Weight.ultraLight)
    containerView.contentView.addSubview(nameLabel)
    buyButton.setTitle("Get Now", for: .normal)
    buyButton.titleLabel?.font = UIFont.systemFont(ofSize: 8, weight: UIFont.Weight.bold)
    ...
    containerView.contentView.addSubview(buyButton)
  }
